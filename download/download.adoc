...
* #{site.pom.latest.droolsWorkbenchTomcatWAR}[Tomcat 7+ WAR]:
  The current build is not deployable on Tomcat, please use Wildfly for now.

* Firefox/IE: The process designer is not working properly on FireFox neither on IE.
  Please use Chrome for now.
